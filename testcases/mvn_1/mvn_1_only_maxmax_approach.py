...
pos=ax1[0].matshow((inverse_model_sensitivity.sensitivity_matrix.todense().transpose()),cmap='RdBu',aspect='auto')
...
pos=ax1[1].matshow(np.abs(inverse_model_sensitivity.sensitivity_matrix.todense().transpose()),aspect='auto', norm=mpl.colors.LogNorm())
...
pos2=ax1[2].matshow(inverse_model_sensitivity.param_sensitivity.reshape(1,-1),aspect='3', norm=mpl.colors.LogNorm())
fig1.colorbar(pos2,ax=ax1[2],location="bottom")

fig1.savefig("output/realistic_box_200_only_maxmax_init_0/sensitivity_matrix.png", dpi=200)
...
#for nr_of_meas in range(1, 9):
    ...
    for realisation in range(nr_of_realisations):
        ...
        # Find the edge with max sensitivity for each boundary. Order boundaries according to sensitivities:

        sensitivity_matrix = inverse_model_sensitivity.sensitivity_matrix

        max_sensitivities = np.array(np.max(sensitivity_matrix.todense(), axis=0)).reshape(-1)
        max_sensitivities_eids = np.array(np.argmax(sensitivity_matrix.todense(), axis=0)).reshape(-1)

        eids_sensmatrix_order = np.flip(max_sensitivities_eids[np.argsort(max_sensitivities)])
        eid_meas_maxmax_approach = eids_sensmatrix_order[:nr_of_meas]

        k = 0
        while np.size(np.unique(eid_meas_maxmax_approach)) != nr_of_meas:
            eid_meas_maxmax_approach = eids_sensmatrix_order[:(nr_of_meas+k)]
            k+=1


        eid_meas_maxmax_approach = eids_sensmatrix_order[:nr_of_meas]  # Max-max approach

        eid_max_sensitivity = np.argmax(inverse_model_sensitivity.param_sensitivity)

        inverse_model_sens_approach.edge_constraint_eid = np.array([eid_meas_maxmax_approach]).reshape(-1)
        ...
        inverse_model_sens_approach.edge_constraint_value = np.array([flowrate_ground_truth[eid_meas_maxmax_approach]]).reshape(-1)
    ...
    with open('output/realistic_box_200_only_maxmax_init_0/norm_flowrate_to_truth_sens.json', 'w') as fp:
        json.dump(norm_flowrate_to_truth_sens, fp)

    with open('output/realistic_box_200_only_maxmax_init_0/norm_boundary_to_truth_sens.json', 'w') as fp:
