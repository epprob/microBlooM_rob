...
nr_of_realisations = 11

for nr_of_meas in range(21, 29):
#for nr_of_meas in range(1, 9):

    flow_rates_opt_sensitivity = np.zeros((flow_network_small_ref.nr_of_es, nr_of_realisations))
    flow_rates_opt_random = np.zeros((flow_network_small_ref.nr_of_es, nr_of_realisations))
    ...
    for realisation in range(nr_of_realisations):
        ...
        flow_network_rand_approach.nr_of_vs = np.copy(flow_network_small_ref.nr_of_vs)
        flow_network_rand_approach.nr_of_es = np.copy(flow_network_small_ref.nr_of_es)
        flow_network_rand_approach.length = np.copy(flow_network_small_ref.length)
        flow_network_rand_approach.diameter = np.copy(flow_network_small_ref.diameter)
        flow_network_rand_approach.flow_rate = np.copy(flow_network_small_ref.flow_rate)
        flow_network_rand_approach.rbc_velocity = np.copy(flow_network_small_ref.rbc_velocity)
        flow_network_rand_approach.edge_list = np.copy(flow_network_small_ref.edge_list)
        flow_network_rand_approach.xyz = np.copy(flow_network_small_ref.xyz)
        flow_network_rand_approach.pressure = np.copy(flow_network_small_ref.pressure)
        flow_network_rand_approach.boundary_vs = np.copy(flow_network_small_ref.boundary_vs)
        flow_network_rand_approach.boundary_val = np.copy(flow_network_small_ref.boundary_val)
        flow_network_rand_approach.boundary_type = np.copy(flow_network_small_ref.boundary_type)
        print("Read network: DONE")
        ...
        j = 0
        while inverse_model_sens_approach.f_h > (1e-4 * nr_of_meas) and j < 30001:
        ...
        rng = default_rng()
        numbers = rng.choice(flow_network_rand_approach.nr_of_es, size=nr_of_meas, replace=False)
        ...
        print("Solve the inverse problem and update the boundaries (Random approach): ...")
        j = 0
        while inverse_model_rand_approach.f_h > (1e-4 * nr_of_meas) and j < 30001:
        ...
        print("Solve the inverse problem and update the boundaries (Random approach): DONE")
...
with open('output/realistic_box_200_std_100_v3/norm_flowrate_to_truth_sens.json', 'w') as fp:
    json.dump(norm_flowrate_to_truth_sens, fp)

with open('output/realistic_box_200_std_100_v3/norm_flowrate_to_truth_rand.json', 'w') as fp:
    json.dump(norm_flowrate_to_truth_rand, fp)

with open('output/realistic_box_200_std_100_v3/norm_boundary_to_truth_sens.json', 'w') as fp:
    json.dump(norm_boundary_to_truth_sens, fp)

with open('output/realistic_box_200_std_100_v3/norm_boundary_to_truth_rand.json', 'w') as fp:
